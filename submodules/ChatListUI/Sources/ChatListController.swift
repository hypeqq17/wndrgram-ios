...
public class ChatListControllerImpl: TelegramBaseController, ChatListController {
    ...
    private func reloadFilters(firstUpdate: (() -> Void)? = nil) {
        ...
        |> deliverOnMainQueue).startStrict(next: { [weak self] countAndFilterItems, peerView, limits in
            ...
            if AyuSettings.current.hideAllChatsFolder && resolvedItems.count > 1 {
                // Only drop the tab, never the underlying filter: `.all` stays
                // available as a destination, and with no folders configured
                // the strip would otherwise be empty.
                resolvedItems.removeAll(where: { $0.id == .all })
            }
