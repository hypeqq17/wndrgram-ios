...
/// Ghost mode splits input activities in two: plain "typing"-style hints and
/// upload progress. Each has its own switch, so the gate is per-activity.
private func ayuAllowsActivity(_ activity: PeerInputActivity?) -> Bool {
    let settings = AyuSettings.current
    guard let activity = activity else {
        // A cancel action carries no information the user wants to hide, and
        // dropping it would leave a stale "typing..." on the other side.
        return true
    }
    switch activity {
    case .uploadingFile, .uploadingPhoto, .uploadingVideo, .uploadingInstantVideo:
        return settings.sendUploadProgress
    case .speakingInGroupCall:
        // The call itself already reveals participation; muting this would
        // only break the group call UI.
        return true
    default:
        return settings.sendTypingStatus
    }
}
