...
/// Marking a dialog *unread* is a deliberate user action and still has to
/// reach the server; only the read receipt itself is suppressed.
private func ayuReadStateOnlyMarksUnread(_ readState: PeerReadState) -> Bool {
    if case let .idBased(_, _, _, _, markedUnread) = readState, markedUnread {
        return true
    }
    return false
}
...
private func pushPeerReadState(network: Network, postbox: Postbox, stateManager: AccountStateManager, peerId: PeerId, readState: PeerReadState) -> Signal<PeerReadState, PeerReadStateValidationError> {
    if !AyuSettings.current.sendReadMessages && !ayuReadStateOnlyMarksUnread(readState) {
        // Report success so the sync loop clears the pending marker instead of
        // retrying forever; the read state is already applied locally.
