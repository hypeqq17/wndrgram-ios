...
private class AdMessagesHistoryContextImpl {
    ...
    init(queue: Queue, account: Account, peerId: EnginePeer.Id, messageId: EngineMessage.Id?, activateManually: Bool) {
        ...
        if messageId == nil && !AyuSettings.current.disableAds {
    ...
    func activate() {
        ...
        if AyuSettings.current.disableAds {
            // Not just hidden: the sponsored-messages request is never made, so
            // the ad server learns nothing about which channels are opened.
