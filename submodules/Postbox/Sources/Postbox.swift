...
public final class Transaction {
    ...
    public func deleteMessages(_ messageIds: [MessageId], forEachMedia: ((Media) -> Void)?) {
        assert(!self.disposed)
        PostboxMessageDeletionHook.willDelete?(self, messageIds)
        let idsToDelete = PostboxMessageDeletionHook.filterDeletions?(self, messageIds) ?? messageIds
        if idsToDelete.isEmpty {
            return
        }
        self.postbox?.deleteMessages(idsToDelete, forEachMedia: forEachMedia)
    }
    ...
    public func deleteMessagesWithGlobalIds(_ ids: [Int32], forEachMedia: ((Media) -> Void)?) {
        ...
        if let postbox = self.postbox {
            ...
            postbox.deleteMessages(idsToDelete, forEachMedia: forEachMedia)
        }
    }
